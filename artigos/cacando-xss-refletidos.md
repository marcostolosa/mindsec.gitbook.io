...
O primeiro passo é identificar onde o seu input é refletido na resposta da aplicação. Insira uma string única (como `intigriti337test`) em vários campos de input, parâmetros de URL, cabeçalhos ou qualquer outro ponto de dados controlável pelo utilizador.
...
Uma vez encontrado um ponto de reflexão, teste se consegue “sair” do contexto atual injetando uma simples tag HTML como `<s>intigriti1337test` e observar como a aplicação a lida.
